...
public class RootLayoutController implements LanguageInterface {
	...
	private void handleLogoutClick(){
		if(mainApp.getController().logout()){
			...
			Optional<ButtonType> result = UtilityClass.showAlert(AlertType.CONFIRMATION, "Wylogowanie", "Potwierdzenie", "Czy chcesz wylogowaæ siê z programu?", ownerStage).showAndWait();
		    if(result.get() == ButtonType.OK){	
		    	//user clicked OK
		    	UtilityClass.showAlert(AlertType.INFORMATION, "Wylogowanie", "Wylogowanie", "Pomyœlnie wylogowano z programu", ownerStage).showAndWait();
		 	    } 
	...
	private boolean showMyProfileDialog(User user){
		MyProfileOverviewController controller = (MyProfileOverviewController) UtilityClass.showDialogFactory("view/MyProfileOverview.fxml", dialogStage, mainApp, mainApp.getPrimaryStage(), "Mój profil");
		controller.setUser(user);
		controller.getCurrentStage().setTitle("Mój profil");
	...
	private void showDevelopersDialog(){
		DevelopersDialogController controller = (DevelopersDialogController) UtilityClass.showDialogFactory("view/menuItemDialogs/DevelopersDialog.fxml", dialogStage, mainApp, ownerStage, "Twórcy");
		controller.getCurrentStage().showAndWait();
	}
